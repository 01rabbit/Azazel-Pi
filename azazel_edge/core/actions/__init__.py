...
新しい統合システムでは azazel_edge.core.enforcer.traffic_control.TrafficControlEngine を使用してください。
テスト用途以外での使用は推奨されません。
"""

import warnings
warnings.warn(
    "azazel_edge.core.actions モジュールは非推奨です。traffic_control.TrafficControlEngine を使用してください。",
    DeprecationWarning,
    stacklevel=2
)
