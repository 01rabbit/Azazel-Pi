...
Azazel-Piは包括的なターミナルベースのメニューインターフェースを提供します：
...
1. **防御制御** - 現在の状態表示、手動モード切り替え、決定履歴の確認
2. **サービス管理** - Azazelサービスの開始/停止/再起動、ログ表示
3. **ネットワーク情報** - インターフェース状態、アクティブプロファイル、トラフィック統計
4. **ログ監視** - リアルタイムログモニタリング、アラート要約
5. **システム情報** - リソース使用状況、温度監視、システム負荷
6. **緊急操作** - 緊急ロックダウン、システムリセット、状態レポート生成
